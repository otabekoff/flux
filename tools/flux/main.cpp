...
DriverOptions parseArgs(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--help" || arg == "-h") {
            ...
            if (fmt == "llvm-ir") opts.outputFormat = flux::OutputFormat::LLVMIR;
            else if (fmt == "bitcode") opts.outputFormat = flux::OutputFormat::Bitcode;
            else if (fmt == "asm") opts.outputFormat = flux::OutputFormat::Assembly;
            else if (fmt == "obj") opts.outputFormat = flux::OutputFormat::Object;
            else if (fmt == "exe") opts.outputFormat = flux::OutputFormat::Executable;
            else {
...
std::string deriveOutputFilename(const std::string& input,
    ...
    switch (format) {
    case flux::OutputFormat::LLVMIR:     return stem + ".ll";
    case flux::OutputFormat::Bitcode:    return stem + ".bc";
    case flux::OutputFormat::Assembly:   return stem + ".s";
    case flux::OutputFormat::Object:     return stem + ".o";
...
int main(int argc, char* argv[]) {
    ...
    if (opts.dumpTokens) {
        ...
        for (auto& tok : tokens) {
            std::cout << flux::Token::kindToString(tok.kind)
                      << " '" << tok.text << "'"
                      << " @ " << tok.location.line
                      << ":" << tok.location.column << "\n";
        }
        if (diag.getErrorCount() > 0) return 1;
    ...
    std::string outFile = opts.outputFile.empty()
