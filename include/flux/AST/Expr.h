...
protected:
    explicit Expr(Kind k, SourceLocation loc = {})
        : ASTNode(loc), kind(k) {}
};
...
public:
    ...
    CastExpr(ExprPtr e, TypeNodePtr type, SourceLocation loc = {})
        : Expr(Kind::Cast, loc), expr(std::move(e)),
...
public:
    ...
    RangeExpr(ExprPtr start, ExprPtr end, bool inclusive = false,
              SourceLocation loc = {})
        : Expr(Kind::Range, loc), start(std::move(start)),
...
public:
    ...
    CompoundAssignExpr(CompoundAssignOp op, ExprPtr target, ExprPtr value,
                       SourceLocation loc = {})
        : Expr(Kind::CompoundAssign, loc), op(op),
