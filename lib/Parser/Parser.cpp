...
ast::DeclPtr Parser::parseDeclaration() {
    ...
    if (check(TokenKind::KwFunc))   return parseFuncDecl();
    if (check(TokenKind::KwAsync)) {
        advance();
        return parseFuncDecl(/*isAsync=*/true);
    }
    if (check(TokenKind::KwStruct)) return parseStructDecl();
    if (check(TokenKind::KwClass))  return parseClassDecl();
    if (check(TokenKind::KwEnum))   return parseEnumDecl();
    if (check(TokenKind::KwTrait))  return parseTraitDecl();
    if (check(TokenKind::KwImpl))   return parseImplDecl();
    if (check(TokenKind::KwType))   return parseTypeAliasDecl();
    ...
    // Not a declaration — try statement (for top-level let/const or function body)
...
std::vector<ast::GenericParam> Parser::parseGenericParams() {
    ...
    if (!match(TokenKind::Less)) return params;
    ...
    while (!check(TokenKind::Greater) && !check(TokenKind::Eof)) {
        ...
        if (check(TokenKind::Apostrophe)) {
            ...
            auto nameTok = expect(TokenKind::Identifier, "expected type parameter name");
...
std::unique_ptr<ast::StructDecl> Parser::parseStructDecl() {
    ...
    auto decl = std::make_unique<ast::StructDecl>(
        std::move(name), std::move(fields), loc);
...
std::unique_ptr<ast::EnumDecl> Parser::parseEnumDecl() {
    ...
    auto decl = std::make_unique<ast::EnumDecl>(
        std::move(name), std::move(variants), loc);
    decl->genericParams = std::move(genericParams);
    return decl;
}
...
std::unique_ptr<ast::TraitDecl> Parser::parseTraitDecl() {
    ...
    while (!check(TokenKind::RBrace) && !check(TokenKind::Eof)) {
        ...
        if (check(TokenKind::KwFunc)) {
            ...
            diag_.emitError(current_.location, "expected method declaration in trait");
            advance();
        }
    }
    ...
    auto decl = std::make_unique<ast::TraitDecl>(
        std::move(name), std::move(methods), loc);
    decl->genericParams = std::move(genericParams);
...
std::unique_ptr<ast::ImplDecl> Parser::parseImplDecl() {
    ...
    while (!check(TokenKind::RBrace) && !check(TokenKind::Eof)) {
        ...
        if (check(TokenKind::KwFunc)) {
            ...
            diag_.emitError(current_.location, "expected method declaration in impl block");
            advance();
        }
    }
    ...
    auto decl = std::make_unique<ast::ImplDecl>(
        std::move(targetType), std::move(traitName),
...
std::unique_ptr<ast::TypeAliasDecl> Parser::parseTypeAliasDecl() {
    ...
    auto decl = std::make_unique<ast::TypeAliasDecl>(
        std::move(name), std::move(aliasedType), loc);
    decl->genericParams = std::move(genericParams);
    return decl;
}
...
ast::StmtPtr Parser::parseStatement() {
    if (check(TokenKind::KwLet))      return parseLetStmt();
    if (check(TokenKind::KwConst))    return parseConstStmt();
    if (check(TokenKind::KwReturn))   return parseReturnStmt();
    if (check(TokenKind::KwIf))       return parseIfStmt();
    if (check(TokenKind::KwMatch))    return parseMatchStmt();
    if (check(TokenKind::KwFor))      return parseForStmt();
    if (check(TokenKind::KwWhile))    return parseWhileStmt();
    if (check(TokenKind::KwLoop))     return parseLoopStmt();
...
std::unique_ptr<ast::LetStmt> Parser::parseLetStmt() {
    ...
    expect(TokenKind::Colon, "expected ':' after variable name (Flux requires explicit types)");
    auto type = parseType();
    ...
    return std::make_unique<ast::LetStmt>(
        std::move(name), std::move(type), std::move(init), isMutable, loc);
}
...
std::unique_ptr<ast::ConstStmt> Parser::parseConstStmt() {
    ...
    expectSemicolon();
    return std::make_unique<ast::ConstStmt>(
        std::move(name), std::move(type), std::move(value), loc);
}
...
std::unique_ptr<ast::MatchStmt> Parser::parseMatchStmt() {
    ...
    return std::make_unique<ast::MatchStmt>(
        std::move(scrutinee), std::move(arms), loc);
}
...
std::unique_ptr<ast::ForStmt> Parser::parseForStmt() {
    ...
    return std::make_unique<ast::ForStmt>(
        std::move(varName), std::move(varType), std::move(iterable),
        std::move(body), loc);
}
...
ast::ExprPtr Parser::parseAssignment() {
    ...
    if (check(TokenKind::Equal)) {
        ...
        return std::make_unique<ast::AssignExpr>(
            std::move(expr), std::move(value), loc);
    }

    // Compound assignment
    auto compoundOp = [&]() -> std::optional<ast::CompoundAssignOp> {
        switch (current_.kind) {
            case TokenKind::PlusEqual:      return ast::CompoundAssignOp::AddAssign;
            case TokenKind::MinusEqual:     return ast::CompoundAssignOp::SubAssign;
            case TokenKind::StarEqual:      return ast::CompoundAssignOp::MulAssign;
            case TokenKind::SlashEqual:     return ast::CompoundAssignOp::DivAssign;
            case TokenKind::PercentEqual:   return ast::CompoundAssignOp::ModAssign;
            case TokenKind::AmpersandEqual: return ast::CompoundAssignOp::AndAssign;
            case TokenKind::PipeEqual:      return ast::CompoundAssignOp::OrAssign;
            case TokenKind::CaretEqual:     return ast::CompoundAssignOp::XorAssign;
...
ast::ExprPtr Parser::parseOr() {
    ...
    while (check(TokenKind::KwOr)) {
        ...
        left = std::make_unique<ast::BinaryExpr>(
            ast::BinaryOp::Or, std::move(left), std::move(right), loc);
    }
    return left;
}
...
ast::ExprPtr Parser::parseEquality() {
    ...
    while (check(TokenKind::EqualEqual) || check(TokenKind::BangEqual)) {
        auto loc = current_.location;
        auto op = current_.kind == TokenKind::EqualEqual
            ? ast::BinaryOp::Equal : ast::BinaryOp::NotEqual;
        advance();
        auto right = parseComparison();
        left = std::make_unique<ast::BinaryExpr>(
            op, std::move(left), std::move(right), loc);
    }
    return left;
}
...
ast::ExprPtr Parser::parseComparison() {
    ...
    while (check(TokenKind::Less) || check(TokenKind::LessEqual) ||
        ...
        switch (current_.kind) {
            ...
            default: op = ast::BinaryOp::Less; break; // unreachable
        }
        advance();
...
ast::ExprPtr Parser::parseMultiplicative() {
    ...
    while (check(TokenKind::Star) || check(TokenKind::Slash) || check(TokenKind::Percent)) {
        ...
        switch (current_.kind) {
            ...
            default: op = ast::BinaryOp::Mul; break;
        }
        advance();
...
ast::ExprPtr Parser::parseUnary() {
    ...
    if (match(TokenKind::Minus)) {
        ...
        return std::make_unique<ast::UnaryExpr>(
            ast::UnaryOp::Negate, std::move(operand), loc);
    }
    ...
    if (match(TokenKind::KwNot)) {
        auto operand = parseUnary();
        return std::make_unique<ast::UnaryExpr>(
            ast::UnaryOp::Not, std::move(operand), loc);
    }
    ...
    if (match(TokenKind::Tilde)) {
        auto operand = parseUnary();
        return std::make_unique<ast::UnaryExpr>(
            ast::UnaryOp::BitwiseNot, std::move(operand), loc);
    }
...
ast::ExprPtr Parser::parsePostfix(ast::ExprPtr left) {
    ...
    if (match(TokenKind::Dot)) {
        auto loc = current_.location;
        auto memberTok = expect(TokenKind::Identifier, "expected member name after '.'");
        ...
        return std::make_unique<ast::MemberAccessExpr>(
            std::move(left), std::move(member), loc);
    }
    ...
    if (check(TokenKind::ColonColon)) {
        ...
        advance();
        auto loc = current_.location;
        auto nextTok = expect(TokenKind::Identifier, "expected identifier after '::'");
        ...
        while (match(TokenKind::ColonColon)) {
            auto seg = expect(TokenKind::Identifier, "expected identifier after '::'");
    ...
    if (match(TokenKind::KwAs)) {
        ...
        return std::make_unique<ast::CastExpr>(
            std::move(left), std::move(type), left->location);
    }
...
ast::ExprPtr Parser::parseCallArguments(ast::ExprPtr callee) {
    ...
    return std::make_unique<ast::CallExpr>(
        std::move(callee), std::move(args), loc);
}
...
ast::ExprPtr Parser::parseIndexExpression(ast::ExprPtr object) {
    ...
    return std::make_unique<ast::IndexExpr>(
        std::move(object), std::move(index), loc);
}
...
ast::ExprPtr Parser::parsePrimary() {
    ...
    diag_.emitError(loc,
        "expected expression, got '" + std::string(current_.text) + "'");
    return nullptr;
}
...
ast::PatternPtr Parser::parsePattern() {
    ...
    if (check(TokenKind::Identifier)) {
        ...
        if (check(TokenKind::ColonColon)) {
            ...
            auto pattern = std::make_unique<ast::ConstructorPattern>(std::move(path), loc);
...
ast::TypeNodePtr Parser::parseNamedOrGenericType() {
    ...
    if (check(TokenKind::Less)) {
        ...
        return std::make_unique<ast::GenericType>(
            std::move(base), std::move(typeArgs), loc);
    }
...
ast::TypeNodePtr Parser::parseTupleType() {
    ...
    if (match(TokenKind::Arrow)) {
        ...
        return std::make_unique<ast::FunctionType>(
            std::move(elements), std::move(retType), loc);
    }
