...
IREmitter::IREmitter(llvm::LLVMContext& ctx, llvm::Module& module,
    ...
    : ctx_(ctx), module_(module), builder_(ctx), diag_(diag),
      typeMapper_(ctx) {}
...
void IREmitter::emitFuncDecl(ast::FuncDecl& decl) {
    ...
    llvm::Type* retType = decl.returnType
        ? typeMapper_.mapType(*decl.returnType)
    ...
    auto* func = llvm::Function::Create(
        funcType, linkage, decl.name, &module_);
...
llvm::Value* IREmitter::emitFloatLiteral(ast::FloatLiteralExpr& expr) {
    return llvm::ConstantFP::get(
        llvm::Type::getDoubleTy(ctx_), expr.value);
}
...
llvm::Value* IREmitter::emitBoolLiteral(ast::BoolLiteralExpr& expr) {
    return llvm::ConstantInt::get(
        llvm::Type::getInt1Ty(ctx_), expr.value ? 1 : 0);
}
